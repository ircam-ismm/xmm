...
#pragma mark === Public Interface ===
    ...
    /**
     * @brief Default value for the Incremental Learning update of the
     * transition matrix
     */
...
#pragma mark
    ...
    /**
     * @brief Get the number of Gaussian mixture components of the observation
    ...
    /**
     * @brief Set the number of Gaussian mixture components of the observation
     * probability distribution
     * @param nbMixtureComponents number of Gaussian mixture components
     * @throws invalid_argument if the number of Gaussian mixture components is
     * <= 0
     */
    void set_nbMixtureComponents(int nbMixtureComponents);

    /**
     * @brief Get Offset added to covariance matrices for convergence (Relative
     * to data variance)
    ...
    /**
     * @brief Get Offset added to covariance matrices for convergence (Minimum
     * value)
     * @return Offset added to covariance matrices for convergence
     */
     ...
     * @param varianceOffset_relative offset to add to the diagonal of
     * covariance matrices (relative to data variance)
     * @param varianceOffset_absolute offset to add to the diagonal of
     * covariance matrices (minimum value)
     * @throws invalid_argument if the covariance offset is <= 0
     */
    void set_varianceOffset(double varianceOffset_relative,
     ...
     * @see REGRESSION_ESTIMATOR
     */
    void set_regression_estimator(
        HMM::REGRESSION_ESTIMATOR regression_estimator);

    /**
     * @brief get transition mode of the hidden Markov Chain
     * @return string corresponding to the transition mode (left-right /
     * ergodic)
     */
     ...
     * @param transMode_str string keyword corresponding to the transition mode
     * ("left-right" / "ergodic")
     ...
     * @throws invalid_argument if the array has a wrong format (not enough
     * values)
     ...
     * @param proba probability of making a transition from srcSegmentLabel to
     * dstSegmentLabel
     * @warning transitions are normalized after the value is set.
     * @todo absolute/relative mode?
     */
    void setOneTransition(Label srcSegmentLabel, Label dstSegmentLabel,
                          double proba);
...
#pragma mark > Performance
     ...
     * @param observation observation vector. If the model is bimodal, this
     * should be allocated for
     * both modalities, and should contain the observation on the input
     * modality. The predicted
...
#pragma mark > Conversion & Extraction
     ...
     * @throws out_of_range if the number or indices of the requested columns
     * exceeds the current dimension
     * @return a HierarchicalHMM from the current model considering only the
     * target columns
     */
     ...
     * @return a unimodal GMMGroup of the input modality from the current
     * bimodal model
     */
     ...
     * @throws runtime_error if the model is training or if it is not bimodal
     * @return a unimodal HierarchicalHMM of the output modality from the
     ...
     * @throws runtime_error if the model is training or if it is not bimodal
     * @return a bimodal HierarchicalHMM  that swaps the input and output
     * modalities
     */
...
#pragma mark === Public attributes ===
     ...
     * to the root)
     */
    std::vector<double> exitTransition;
...
#pragma mark > Forward Algorithm
     ...
     * modality. The predicted
     * output will be appended to the input modality observation
     * @todo check if the algorithm is right for an ergodic transition structure
     * of the sub-hmms
     */

...
void xmm::KMeans::train(TrainingSet *trainingSet)
{
    ...
    centers.resize(configuration.clusters.get() * shared_parameters->dimension.get(), 0.0);
    ...
    for (int trainingNbIterations=0; trainingNbIterations<configuration.max_iterations.get(); ++trainingNbIterations)
    {
        ...
        for (std::size_t k=0; k<clusters; ++k) {
            for (std::size_t l=0; l<clusters; ++l) {
                if (k != l) {
                    meanClusterDistance += euclidean_distance(&centers[k*dimension], &centers[l*dimension], dimension);
                }
            }
            maxRelativeCenterVariation = kmax(euclidean_distance(&previous_centers[k*dimension], &centers[k*dimension], dimension),
        ...
        if (maxRelativeCenterVariation < configuration.relative_distance_threshold.get())
            break;
    }
}

void xmm::KMeans::initClustersWithFirstPhrase(Phrase *phrase)
{
    int dimension = static_cast<int>(shared_parameters->dimension.get());
    ...
    for (std::size_t c=0; c<configuration.clusters.get(); c++) {
        ...
        for (std::size_t t=0; t<step; t++) {
            for (std::size_t d=0; d<dimension; d++) {
                centers[c*dimension+d] += phrase->getValue(offset+t, d) / float(step);
            }
        }
        offset += step;
    }
...
void xmm::KMeans::randomizeClusters(std::vector<float> const& trainingSetVariance)
{
    ...
    for (std::size_t k=0; k<configuration.clusters.get(); ++k) {
        for (std::size_t d=0; d<dimension; ++d) {
            centers[k*dimension + d] = trainingSetVariance[d] * (2. * random() / float(RAND_MAX) - 1.);
        }
    }
}

void xmm::KMeans::updateCenters(std::vector<float>& previous_centers, TrainingSet *trainingSet)
{
    int dimension = static_cast<int>(shared_parameters->dimension.get());
    int clusters = static_cast<int>(configuration.clusters.get());
    ...
    for (auto it=trainingSet->begin(); it!=trainingSet->end(); ++it, ++phraseIndex) {
        for (std::size_t t=0; t<it->second->size(); ++t) {
            ...
            if (trainingSet->bimodal()) {
                ...
                min_distance = euclidean_distance(&frame[0],
                                                  &previous_centers[0],
                                                  dimension);
            } else {
                min_distance = euclidean_distance(it->second->getPointer(t),
                                                  &previous_centers[0],
                                                  dimension);
            }
            ...
            for (std::size_t k=1; k<clusters; ++k) {
                ...
                if (trainingSet->bimodal()) {
                    ...
                    distance = euclidean_distance(&frame[0],
                                                  &previous_centers[k*dimension],
                                                  dimension);
                } else {
                    distance = euclidean_distance(it->second->getPointer(t),
                                                  &previous_centers[k*dimension],
                                                  dimension);
                }
            ...
            for (std::size_t d=0; d<dimension; ++d) {
                centers[cluster_membership * dimension + d] += it->second->getValue(t, d);
            }
        }
    }
    for (std::size_t k=0; k<clusters; ++k) {
...
void xmm::KMeans::filter(std::vector<float> const& observation)
{
    ...
    for (std::size_t k=0; k<configuration.clusters.get(); ++k) {
        results.distances[k] = euclidean_distance(&observation[0], &centers[k*dimension], dimension);

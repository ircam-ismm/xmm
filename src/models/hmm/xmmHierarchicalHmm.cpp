...
Model<SingleClassHMM, HMM>(bimodal),
...
void xmm::HierarchicalHMM::updateTransition()
{
    ...
    for (auto &srcModel : models)
    {
        ...
        for (auto &dstModel : models)
        {
            transition[srcModel.first][dstModel.first] = 1. / static_cast<double>(this->size());
        }
    }
}
...
void xmm::HierarchicalHMM::forward_init(std::vector<float> const& observation)
{
    ...
    for (auto &model : models)
    {
        ...
        if (model.second.parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
            for (int i=0 ; i<model.second.parameters.states.get(); i++) {
                if (shared_parameters->bimodal.get()) {
                    model.second.alpha_h[0][i] = model.second.prior[i] * model.second.states[i].obsProb_input(&observation[0]);
                } else {
                    model.second.alpha_h[0][i] = model.second.prior[i] * model.second.states[i].obsProb(&observation[0]);
                }
                model.second.results.instant_likelihood += model.second.alpha_h[0][i] ;
            }
        } else {
            model.second.alpha_h[0][0] = this->prior[model.first];
            if (shared_parameters->bimodal.get()) {
                model.second.alpha_h[0][0] *= model.second.states[0].obsProb_input(&observation[0]);
            } else {
                model.second.alpha_h[0][0] *= model.second.states[0].obsProb(&observation[0]);
            }
            model.second.results.instant_likelihood = model.second.alpha_h[0][0] ;
        }
...
void xmm::HierarchicalHMM::forward_update(std::vector<float> const& observation)
{
    ...
    for (auto &dstModel : models)
    {
        ...
        if (dstModel.second.parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
            for (int k=0 ; k<N ; ++k)
            {
                for (unsigned int j=0; j<N; ++j) {
                    front[k] += dstModel.second.transition[j*N + k] / (1 - dstModel.second.exit_probabilities_[j]) * dstModel.second.alpha_h[0][j] ;
                }
                
                int i(0);
                for (auto &srcModel : models)
                {
                    front[k] += dstModel.second.prior[k] * (frontier_v1_[i] * this->transition[srcModel.first][dstModel.first] + this->prior[dstModel.first] * frontier_v2_[i]);
                    i++;
                }
            }
        } else {
            // k=0: first state of the primitive
            front[0] = dstModel.second.transition[0] * dstModel.second.alpha_h[0][0] ;
            
            int i(0);
            for (auto &srcModel : models)
            {
                front[0] += frontier_v1_[i] * this->transition[srcModel.first][dstModel.first] + this->prior[dstModel.first] * frontier_v2_[i];
                i++;
            }
            ...
            for (int k=1 ; k<N ; ++k)
            {
                front[k] += dstModel.second.transition[k*2] / (1 - dstModel.second.exit_probabilities_[k]) * dstModel.second.alpha_h[0][k] ;
                front[k] += dstModel.second.transition[(k-1)*2+1] / (1 - dstModel.second.exit_probabilities_[k-1]) * dstModel.second.alpha_h[0][k-1] ;
            }
        ...
        {
            if (shared_parameters->bimodal.get())
                tmp = dstModel.second.states[k].obsProb_input(&observation[0]) * front[k];
            else
                tmp = dstModel.second.states[k].obsProb(&observation[0]) * front[k];
            
            dstModel.second.alpha_h[2][k] = this->exit_transition[dstModel.first] * dstModel.second.exit_probabilities_[k] * tmp ;
            dstModel.second.alpha_h[1][k] = (1 - this->exit_transition[dstModel.first]) * dstModel.second.exit_probabilities_[k] * tmp ;
            dstModel.second.alpha_h[0][k] = (1 - dstModel.second.exit_probabilities_[k]) * tmp;
            
            dstModel.second.results.exit_likelihood += dstModel.second.alpha_h[1][k] +dstModel.second.alpha_h[2][k];
            dstModel.second.results.instant_likelihood += dstModel.second.alpha_h[0][k] + dstModel.second.alpha_h[1][k] + dstModel.second.alpha_h[2][k];
            
            norm_const += tmp;
        }
        
        dstModel.second.results.exit_ratio = dstModel.second.results.exit_likelihood / dstModel.second.results.instant_likelihood;
    }
    ...
    if (exitNum<0) { // Likelihood over all exit states
        ...
        for (auto &model : models) {
            ...
            for (std::size_t exit = 0; exit<3; ++exit) {
                for (std::size_t k=0; k<model.second.parameters.states.get(); k++){
...
void xmm::HierarchicalHMM::filter(std::vector<float> const& observation)
{
    ...
    if (shared_parameters->bimodal.get()) {
        ...
        for (auto &model : models) {
            model.second.regression(observation, model.second.results.output_values);
        }
        ...
        if (configuration.multiClass_regression_estimator == MultiClassRegressionEstimator::Likeliest) {
            ...
            for (auto &model : models) {
                for (int d=0; d<dimension_output; d++) {
                    // TODO: Same here
                    results.output_values[d] += results.instant_normalized_likelihoods[i] * model.second.results.output_values[d];
                    results.output_variance[d] += results.instant_normalized_likelihoods[i] * model.second.results.output_variance[d];
                }
                i++;
            }
        }
...
void xmm::HierarchicalHMM::updateResults()
{
    ...
    int i(0);
    for (auto &model : models) {
        results.instant_likelihoods[i] = model.second.results.instant_likelihood;
        results.smoothed_log_likelihoods[i] = model.second.results.log_likelihood;
...
//}
//
//xmm::HierarchicalHMM xmm::HierarchicalHMM::extractSubmodel(std::vector<std::size_t>& columns) const
//{
//    checkTraining();

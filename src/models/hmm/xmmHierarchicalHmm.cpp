...
void xmm::HierarchicalHMM::updateTransition() {
  ...
  for (int i = 0; i < num_classes; i++) {
    transition[i].assign(num_classes, 1. / static_cast<double>(num_classes));
  }
}
...
void xmm::HierarchicalHMM::forward_update(
  ...
  for (auto &dstModel : models) {
    ...
    if (dstModel.second.parameters.transition_mode.get() ==
      ...
      for (int src_model_index = 0; src_model_index < num_classes;
        ...
        front[0] +=
            ...
            this->prior[dst_model_index] * frontier_v2_[src_model_index];
      }
    ...
    for (int k = 0; k < N; ++k) {
      if (shared_parameters->bimodal.get())
        tmp =
            dstModel.second.states[k].obsProb_input(&observation[0]) * front[k];
      else
        tmp = dstModel.second.states[k].obsProb(&observation[0]) * front[k];

      dstModel.second.alpha_h[2][k] = this->exit_transition[dst_model_index] *
                                      dstModel.second.exit_probabilities_[k] *
                                      tmp;
...
void xmm::HierarchicalHMM::likelihoodAlpha(
  ...
  if (exitNum < 0) {  // Likelihood over all exit states
    ...
    for (auto &model : models) {
      likelihoodVector[l] = 0.0;
      for (unsigned int k = 0; k < model.second.parameters.states.get(); k++) {

...
def get_model(model_group, label):
    ...
    """
    return model_group.models[mhmm.Label(label)]
...
def compute_ellipse(mean, covariance, axis1, axis2):
    ...
    print mean2d[0], mean2d[1], ellipse_mhmm.widthsqrt(chi2.ppf(0.95, 2)*eigenVal[1]), theta
...
def gmm_plot_gaussians(gmm, axis1, axis2, axes=None, color='b'):
    ...
    for gaussian in range(num_gaussians):
        ellipse_mhmm = gmm.components[gaussian].ellipse(axis1, axis2)
        ellipses.append(Ellipse(xy=[ellipse_mhmm.x, ellipse_mhmm.y],
                        width=ellipse_mhmm.width,
                        height=ellipse_mhmm.height,
                        angle=ellipse_mhmm.angle*180/np.pi))
    if not axes:
        fig = plt.figure()

...
namespace xmm
{
    /**
     @ingroup Model
     @brief Probabilistic machine learning model for multiclass recognition and regression
    ...
    public:
        ...
        cancel_required_(false),
        models_still_training_(0),
        shared_parameters(std::make_shared<SharedParameters>(*src.shared_parameters)),
        ...
        training_events(src.training_events)
        {
            ...
            for (auto &model : models) {
                ...
                model.second.training_events.addListener(this, &xmm::Model<SingleClassModel, ModelType>::onTrainingEvent);
            }
        }
        ...
        shared_parameters(std::make_shared<SharedParameters>())
        {
            ...
            for (auto p : root["models"]) {
                ...
                models.insert(std::pair<std::string, SingleClassModel>(l, SingleClassModel(shared_parameters, p)));
                models[l].training_events.removeListeners();
                models[l].training_events.addListener(this, &xmm::Model<SingleClassModel, ModelType>::onTrainingEvent);
            }
        }
        ...
        virtual void train(TrainingSet *trainingSet)
        {
            ...
            if (shared_parameters->bimodal.get()) {
                shared_parameters->dimension_input.set(trainingSet->dimension_input.get());
            }
            ...
            for (auto it=this->models.begin(); it != this->models.end(); ++it) {
                ...
                if ((configuration.multithreading == MultithreadingMode::Parallel) ||
                    (configuration.multithreading == MultithreadingMode::Background)) {
                    training_threads_[it->first] = std::thread(&SingleClassModel::train,
                                                               &it->second,
        ...
        /**
         ...
         @throw runtime_error if the dimensions of the training set don't match the dimensions
                           ...
                           std::string const& label)
        {
            ...
            if (size() > 0) {
                if ((shared_parameters->dimension.get() != trainingSet->dimension.get()) ||
                    (shared_parameters->bimodal.get() && (shared_parameters->dimension_input.get() != trainingSet->dimension_input.get())))
                    throw std::runtime_error("Dimensions of the new Training Set do not match existing models");
            }
            ...
            if (configuration.multithreading == MultithreadingMode::Sequential) {
                ...
                training_threads_[label] = std::thread(&SingleClassModel::train,
                                                       &(this->models[label]),
    ...
    protected:
        ...
        virtual void joinTraining()
        {
            ...
            if (is_joining_.compare_exchange_weak(expectedState, true)) {
                ...
                while (classesUntrained) {
                    ...
                    for (auto it = this->models.begin(); it != this->models.end(); it++) {
                        if (it->second.training_status_.status == TrainingEvent::Status::Cancel ||
                            it->second.training_status_.status == TrainingEvent::Status::Error) {
                            models.erase(it);
        ...
        void onTrainingEvent(TrainingEvent const& e)
        {
            ...
            if (e.status != TrainingEvent::Status::Run) {
                ...
                if (configuration.multithreading == MultithreadingMode::Background &&
                    !is_joining_ && // avoid to call "joinTraining" if already called by the main thread
                    !cancel_required_ &&  // avoid to call "joinTraining" if cancel required by the main thread
                    models_still_training_ == 0) {
                    is_joining_ = false;
                    std::thread(&Model<SingleClassModel, ModelType>::joinTraining, this).detach();
                }
            }
            event_mutex_.unlock();
        }
        ...
        /**
         @brief Look for configuration changes and throws an exception if the model is not up to date.
         */
        inline void checkConfigurationChanges() const
        {
            ...
            for (auto &config : configuration.class_parameters_) {
                if (config.second.changed)
                    configChanged = true;
            }
            if (configChanged) {
                throw std::runtime_error("Configuration has changed, models need to be trained.");
            }
        }
        ...
        virtual void addModelForClass(std::string const& label)
        {
            ...
            if (models.find(label) == models.end()) {
                ...
                models[label].training_events.addListener(this, &xmm::Model<SingleClassModel, ModelType>::onTrainingEvent);

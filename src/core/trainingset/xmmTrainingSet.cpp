...
locked_(false)
{
    ...
    dimension_input.onAttributeChange(this, &xmm::TrainingSet::onAttributeChange);
...
labels_(src.labels_)
{
    ...
    for (auto phrase : src.phrases_) {
        phrases_.insert(std::pair<int, Phrase*>(phrase.first, new Phrase(*phrase.second)));
        phrases_[phrase.first]->events.addListener(this, &xmm::TrainingSet::onPhraseEvent);
    }
    update();
}
...
locked_(false)
{
    ...
    for (auto p : root["phrases"]) {
        phrases_.insert(std::pair<int, Phrase*>(p["index"].asInt(), new Phrase(p)));
        phrases_[p["index"].asInt()]->events.addListener(this, &xmm::TrainingSet::onPhraseEvent);
    }
    update();
}
...
std::size_t xmm::TrainingSet::size() const
{
...
void xmm::TrainingSet::onAttributeChange(xmm::AttributeBase *attr_pointer)
{
    if (attr_pointer == &dimension) {
        ...
        for (std::map<std::string, TrainingSet>::iterator it = sub_training_sets_.begin() ; it != sub_training_sets_.end() ; ++it)
...
std::map<int, xmm::Phrase*>::const_reverse_iterator xmm::TrainingSet::crbegin() const
{
    return phrases_.crbegin();
}

std::map<int, xmm::Phrase*>::const_reverse_iterator xmm::TrainingSet::crend() const
{
                                 ...
                                 std::string label)
{
    ...
    phrases_[phraseIndex] = new Phrase(own_memory_ ? MemoryMode::OwnMemory : MemoryMode::SharedMemory,
                                       bimodal_ ? Multimodality::Bimodal : Multimodality::Unimodal);
    phrases_[phraseIndex]->events.addListener(this, &xmm::TrainingSet::onPhraseEvent);
...
{
    std::map<std::string, TrainingSet>::iterator it = sub_training_sets_.find(label);
    if (it == sub_training_sets_.end())
        return nullptr;
    return &(it->second);
}
...
void xmm::TrainingSet::update()
{
    ...
    for (std::string label : labels_) {
        sub_training_sets_[label] = TrainingSet(own_memory_ ? MemoryMode::OwnMemory : MemoryMode::SharedMemory,
                                                bimodal_ ? Multimodality::Bimodal : Multimodality::Unimodal);
        ...
        for (auto &phrase : phrases_) {
            if (phrase.second->label.get() == label) {
                sub_training_sets_[label].phrases_[phrase.first] = this->phrases_[phrase.first];
                //                newPhraseIndex++;
            }
        }
    }
...
std::vector<float> xmm::TrainingSet::variance() const
{
    ...
    for (auto &phrase : phrases_)
    {
        for (unsigned int d=0; d<dimension.get(); d++) {
            for (unsigned int t=0 ; t<phrase.second->size() ; t++) {
                variance[d] += (phrase.second->getValue(t, d) - _mean[d]) * (phrase.second->getValue(t, d) - _mean[d]);
            }
        }
        total_length += phrase.second->size();
    }

...
xmm::SingleClassHMM::SingleClassHMM(std::shared_ptr<SharedParameters> p, Json::Value const& root) :
...
is_hierarchical_(true)
{
    ...
    if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
        ...
        json2vector(root["transition"], transition, parameters.states.get() * parameters.states.get());
    } else {
        json2vector(root["transition"], transition, 2 * parameters.states.get());
        
    }
    if (root.isMember("exitProbabilities")) {
        json2vector(root["exitProbabilities"], exit_probabilities_, parameters.states.get());
    }
...
void xmm::SingleClassHMM::allocate()
{
    ...
    tmpGMM.parameters.relative_regularization.set(parameters.relative_regularization.get());
    tmpGMM.parameters.absolute_regularization.set(parameters.absolute_regularization.get());
    tmpGMM.parameters.covariance_mode.set(parameters.covariance_mode.get());
    states.assign(numStates,
                  tmpGMM);
...
void xmm::SingleClassHMM::initParametersToDefault(std::vector<float> const& dataVariance)
{
    if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
        setErgodic();
    } else {
...
void xmm::SingleClassHMM::initMeansWithAllPhrases(TrainingSet *trainingSet)
{
    ...
    for (auto phrase_it = trainingSet->begin(); phrase_it != trainingSet->end(); phrase_it++) {
        std::size_t step = phrase_it->second->size() / numStates;
        std::size_t offset(0);
        for (std::size_t n=0; n<numStates; n++) {
            for (std::size_t t=0; t<step; t++) {
                for (std::size_t d=0; d<dimension; d++) {
                    states[n].components[0].mean[d] += phrase_it->second->getValue(offset+t, d);
                }
            }
            offset += step;
            factor[n] += step;
...
void xmm::SingleClassHMM::initCovariances_fullyObserved(TrainingSet *trainingSet)
{
    ...
    if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
        for (int n=0; n<numStates; n++)
            states[n].components[0].covariance.assign(dimension*dimension, 0.0);
    } else {
        for (int n=0; n<numStates; n++)
        ...
        for (std::size_t n=0; n<numStates; n++) {
            for (std::size_t t=0; t<step; t++) {
                for (std::size_t d1=0; d1<dimension; d1++) {
                    othermeans[n*dimension+d1] += phrase_it->second->getValue(offset+t, d1);
                    if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                        for (int d2=0; d2<dimension; d2++) {
                            states[n].components[0].covariance[d1*dimension+d2] += phrase_it->second->getValue(offset+t, d1) * phrase_it->second->getValue(offset+t, d2);
                        }
                    } else {
                        float value = phrase_it->second->getValue(offset+t, d1);
    ...
    for (std::size_t n=0; n<numStates; n++)
        for (std::size_t d1=0; d1<dimension; d1++) {
            ...
            if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                for (int d2=0; d2<dimension; d2++)
                    states[n].components[0].covariance[d1*dimension+d2] /= factor[n];
            } else {
    ...
    for (int n=0; n<numStates; n++) {
        for (int d1=0; d1<dimension; d1++) {
            if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                for (int d2=0; d2<dimension; d2++)
                    states[n].components[0].covariance[d1*dimension+d2] -= othermeans[n*dimension+d1]*othermeans[n*dimension+d2];
            } else {
                states[n].components[0].covariance[d1] -= othermeans[n*dimension+d1] * othermeans[n*dimension+d1];
            }
        }
...
void xmm::SingleClassHMM::initMeansCovariancesWithGMMEM(TrainingSet *trainingSet)
{
    ...
    for (std::size_t n=0; n<numStates; n++) {
        TrainingSet temp_ts(MemoryMode::SharedMemory,
                            shared_parameters->bimodal.get() ? Multimodality::Bimodal : Multimodality::Unimodal);
        ...
        for (auto phrase_it = trainingSet->begin() ; phrase_it != trainingSet->end() ; phrase_it++) {
            ...
            if (shared_parameters->bimodal.get())
                phrase_it->second->connect(phrase_it->second->getPointer_input(n*step),
                                           phrase_it->second->getPointer_output(n*step),
                                           step);
            else
                temp_ts.getPhrase(phrase_it->first)->connect(phrase_it->second->getPointer(n*step),
                                                             step);
        }
        SingleClassGMM tmpGMM(shared_parameters);
        tmpGMM.parameters.gaussians.set(parameters.gaussians.get());
        ...
        for (std::size_t c=0; c<parameters.gaussians.get(); c++) {
            ...
            states[n].components[c].covariance = tmpGMM.components[c].covariance;
            states[n].updateInverseCovariances();
        }
    }
}
                                           ...
                                           const float* observation_output)
{
    ...
    for (int j=0; j<numStates; j++) {
        ...
        if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
            ...
            if (j>0) {
                alpha[j] += previous_alpha_[j-1] * transition[(j-1)*2+1];
            } else {
                alpha[0] += previous_alpha_[numStates-1] * transition[numStates*2-1];
            }
        }
        if (shared_parameters->bimodal.get()) {
            if (observation_output)
    ...
    for (int i=0 ; i<numStates; i++) {
        ...
        if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
            for (int j=0; j<numStates; j++) {
                if (shared_parameters->bimodal.get()) {
                    if (observation_output)
                        beta_[i] += transition[i*numStates+j] * previous_beta_[j] * states[j].obsProb_bimodal(observation, observation_output);
                    else
                        beta_[i] += transition[i*numStates+j] * previous_beta_[j] * states[j].obsProb_input(observation);
                } else {
                    beta_[i] += transition[i*numStates+j] * previous_beta_[j] * states[j].obsProb(observation);
                }
            ...
            if (shared_parameters->bimodal.get()) {
                if (observation_output)
                    beta_[i] += transition[i*2] * previous_beta_[i] * states[i].obsProb_bimodal(observation, observation_output);
                else
                    beta_[i] += transition[i*2] * previous_beta_[i] * states[i].obsProb_input(observation);
            } else {
                beta_[i] += transition[i*2] * previous_beta_[i] * states[i].obsProb(observation);
            }
            if (i<numStates-1) {
                if (shared_parameters->bimodal.get()) {
                    if (observation_output)
                        beta_[i] += transition[i*2+1] * previous_beta_[i+1] * states[i+1].obsProb_bimodal(observation, observation_output);
                    else
                        beta_[i] += transition[i*2+1] * previous_beta_[i+1] * states[i+1].obsProb_input(observation);
                } else {
                    beta_[i] += transition[i*2+1] * previous_beta_[i+1] * states[i+1].obsProb(observation);
                }
            }
        }
        beta_[i] *= ct;
...
double xmm::SingleClassHMM::emAlgorithmUpdate(TrainingSet *trainingSet)
{
    ...
    for (int i=0; i<numStates; i++) {
        for (int c=0; c<parameters.gaussians.get(); c++) {
            ...
            if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                states[i].components[c].covariance.assign(dimension * dimension, 0.0);
            } else {
...
void xmm::SingleClassHMM::baumWelch_backward_update(double ct, std::vector<double>::iterator observation_likelihoods)
{
    std::size_t numStates = parameters.states.get();
    ...
    for (int i=0 ; i<numStates; i++) {
        ...
        if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
            for (int j=0; j<numStates; j++) {
                beta_[i] += transition[i*numStates+j] * previous_beta_[j] * observation_likelihoods[j];
            }
        } else {
            beta_[i] += transition[i*2] * previous_beta_[i] * observation_likelihoods[i];
            if (i<numStates-1) {
                beta_[i] += transition[i*2+1] * previous_beta_[i+1] * observation_likelihoods[i+1];
            }
        }
        beta_[i] *= ct;
        if (std::isnan(beta_[i]) || std::isinf(fabs(beta_[i]))) {
...
double xmm::SingleClassHMM::baumWelch_forwardBackward(Phrase* currentPhrase, int phraseIndex)
{
    ...
    for (std::size_t t=0; t<T; ++t) {
        for (std::size_t i=0; i<numStates; i++) {
            if (shared_parameters->bimodal.get()) {
                observation_probabilities[t * numStates + i] = states[i].obsProb_bimodal(currentPhrase->getPointer_input(t),
                                                                                         currentPhrase->getPointer_output(t));
            } else {
                observation_probabilities[t * numStates + i] = states[i].obsProb(currentPhrase->getPointer(t));
            }
        }
    }
    ...
    for (int t=1; t<T; t++) {
        ct[t] = baumWelch_forward_update(observation_probabilities.begin() + t * numStates);
        log_prob -= log(ct[t]);
        copy(alpha.begin(), alpha.end(), alpha_seq_it);
        alpha_seq_it += numStates;
    }
    ...
    for (int t=int(T-2); t>=0; t--) {
        baumWelch_backward_update(ct[t], observation_probabilities.begin() + (t+1) * numStates);
    ...
    for (int t=0; t<T; t++) {
        for (int i=0; i<numStates; i++) {
            gamma_sequence_[phraseIndex][t*numStates+i] = alpha_seq_[t*numStates+i] * beta_seq_[t*numStates+i] / ct[t];
        }
    }
    ...
    for (int t=0; t<T; t++) {
        for (int i=0; i<numStates; i++) {
            ...
            if (parameters.gaussians.get() == 1) {
                ...
                gamma_sequence_per_mixture_[phraseIndex][0][t*numStates+i] = gamma_sequence_[phraseIndex][t*numStates+i] * oo;
                norm_const += oo;
            } else {
                for (int c=0; c<parameters.gaussians.get(); c++) {
                    if (shared_parameters->bimodal.get()) {
                        oo = states[i].obsProb_bimodal(currentPhrase->getPointer_input(t),
                                                       currentPhrase->getPointer_output(t),
                                                       c);
                    } else {
                        oo = states[i].obsProb(currentPhrase->getPointer(t),
                                               c);
                    }
                    gamma_sequence_per_mixture_[phraseIndex][c][t*numStates+i] = gamma_sequence_[phraseIndex][t*numStates+i] * oo;
                    norm_const += oo;
                }
            }
            if (norm_const > 0)
                for (int c=0; c<parameters.gaussians.get(); c++)
                    gamma_sequence_per_mixture_[phraseIndex][c][t*numStates+i] /= norm_const;
        }
    }
    ...
    if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
        for (int t=0; t<T-1; t++) {
            for (int i=0; i<numStates; i++) {
                for (int j=0; j<numStates; j++) {
                    epsilon_sequence_[phraseIndex][t*numStates*numStates+i*numStates+j] = alpha_seq_[t*numStates+i]
                    * transition[i*numStates+j]
                    * beta_seq_[(t+1)*numStates+j];
                    epsilon_sequence_[phraseIndex][t*numStates*numStates+i*numStates+j] *= observation_probabilities[(t+1) * numStates + j];
                }
            }
        }
    } else {
        for (int t=0; t<T-1; t++) {
            for (int i=0; i<numStates; i++) {
                epsilon_sequence_[phraseIndex][t*2*numStates+i*2] = alpha_seq_[t*numStates+i]
                * transition[i*2]
                * beta_seq_[(t+1)*numStates+i];
                epsilon_sequence_[phraseIndex][t*2*numStates+i*2] *= observation_probabilities[(t+1) * numStates + i];
                if (i<numStates-1) {
                    epsilon_sequence_[phraseIndex][t*2*numStates+i*2+1] = alpha_seq_[t*numStates+i]
                    * transition[i*2+1]
                    * beta_seq_[(t+1)*numStates+i+1];
                    epsilon_sequence_[phraseIndex][t*2*numStates+i*2+1] *= observation_probabilities[(t+1) * numStates + i+1];
                }
            }
        }
    }
    ...
    for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); ++it) {
        ...
        for (int i=0; i<numStates; i++) {
            for (int t=0; t<phraseLength; t++) {
                gamma_sum_[i] += gamma_sequence_[phraseIndex][t*numStates+i];
                for (int c=0; c<numGaussians; c++) {
                    gamma_sum_per_mixture_[i*numGaussians+c] += gamma_sequence_per_mixture_[phraseIndex][c][t*numStates+i];
                }
            }
        }
        phraseIndex++;
...
void xmm::SingleClassHMM::baumWelch_estimateMixtureCoefficients(TrainingSet *trainingSet)
{
    ...
    for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); ++it)
    {
        ...
        for (int i=0; i<numStates; i++) {
            for (int t=0; t<phraseLength; t++) {
                for (int c=0; c<parameters.gaussians.get(); c++) {
                    states[i].mixture_coeffs[c] += gamma_sequence_per_mixture_[phraseIndex][c][t*numStates+i];
                }
            }
        }
        phraseIndex++;
    ...
    for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); it++)
    {
        ...
        for (int i=0; i<numStates; i++) {
            for (int t=0; t<phraseLength; t++) {
                for (int c=0; c<numGaussians; c++) {
                    for (int d=0; d<dimension; d++) {
                        states[i].components[c].mean[d] += gamma_sequence_per_mixture_[phraseIndex][c][t*numStates+i] * it->second->getValue(t, d);
                    }
                }
            }
        }
        ...
        for (int c=0; c<numGaussians; c++) {
            for (int d=0; d<dimension; d++) {
                if (gamma_sum_per_mixture_[i*numGaussians+c] > 0) {
                    states[i].components[c].mean[d] /= gamma_sum_per_mixture_[i*numGaussians+c];
                }
                    ...
                    for (int d1=0; d1<dimension; d1++) {
                        if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                            for (int d2=d1; d2<dimension; d2++) {
                                states[i].components[c].covariance[d1*dimension+d2] += gamma_sequence_per_mixture_[phraseIndex][c][t*numStates+i]
                                * (it->second->getValue(t, d1) - states[i].components[c].mean[d1])
                                * (it->second->getValue(t, d2) - states[i].components[c].mean[d2]);
                            }
                        } else {
                            float value = it->second->getValue(t, d1) - states[i].components[c].mean[d1];
                            states[i].components[c].covariance[d1] += gamma_sequence_per_mixture_[phraseIndex][c][t*numStates+i]
                            * value * value;
                        }
                    }
                }
        ...
        for (int c=0; c<numGaussians; c++) {
            if (gamma_sum_per_mixture_[i*numGaussians+c] > 0) {
                for (int d1=0; d1<dimension; d1++) {
                    if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                        for (int d2=d1; d2<dimension; d2++) {
                            states[i].components[c].covariance[d1*dimension+d2] /= gamma_sum_per_mixture_[i*numGaussians+c];
                            if (d1 != d2)
                                states[i].components[c].covariance[d2*dimension+d1] = states[i].components[c].covariance[d1*dimension+d2];
                        }
                    } else {
                        states[i].components[c].covariance[d1] /= gamma_sum_per_mixture_[i*numGaussians+c];
                    }
                }
            }
        }
...
void xmm::SingleClassHMM::baumWelch_estimateTransitions(TrainingSet *trainingSet)
{
    ...
    for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); it++)
    {
        ...
        for (int i=0; i<numStates; i++) {
            ...
            if (parameters.transition_mode.get() == HMM::TransitionMode::LeftRight) {
            ...
            if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
                for (int j=0; j<numStates; j++)
                {
                    for (int t=0; t<phraseLength-1; t++) {
                        transition[i*numStates+j] += epsilon_sequence_[phraseIndex][t*numStates*numStates+i*numStates+j];
                    }
                }
            } else {
                for (int t=0; t<phraseLength-1; t++) {
                    transition[i*2] += epsilon_sequence_[phraseIndex][t*2*numStates+i*2];
                }
                if (i<numStates-1) {
                    for (int t=0; t<phraseLength-1; t++) {
                        transition[i*2+1] += epsilon_sequence_[phraseIndex][t*2*numStates+i*2+1];
                    }
                }
            }
        }
    ...
    if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
        for (int i=0; i<numStates; i++) {
            for (int j=0; j<numStates; j++) {
                transition[i*numStates+j] /= (gamma_sum_[i] + 2.*TRANSITION_REGULARIZATION());
        ...
        for (int i=0; i<numStates; i++) {
            transition[i*2] /= (gamma_sum_[i] + 2.*TRANSITION_REGULARIZATION());
            if (std::isnan(transition[i*2]))
                throw std::runtime_error("Convergence Error. Check your training data or increase the variance offset");
            if (i<numStates-1) {
                transition[i*2+1] /= (gamma_sum_[i] + 2.*TRANSITION_REGULARIZATION());
...
void xmm::SingleClassHMM::addCyclicTransition(double proba)
{
    ...
    if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
        if (numStates > 1)
            transition[(numStates-1)*numStates] = proba;
    } else {
        if (numStates > 1)
            transition[(numStates-1)*2+1] = proba;
    }
}
...
void xmm::SingleClassHMM::updateAlphaWindow()
{
    ...
    // Compute Window
    window_minindex_ = (static_cast<int>(results.likeliest_state) - static_cast<int>(numStates)/2);
    window_maxindex_ = (static_cast<int>(results.likeliest_state) + static_cast<int>(numStates)/2);
    window_minindex_ = (window_minindex_ >= 0) ? window_minindex_ : 0;
    window_maxindex_ = (window_maxindex_ <= static_cast<int>(numStates)) ? window_maxindex_ : static_cast<int>(numStates);
    window_normalization_constant_ = 0.0;
    for (int i=window_minindex_; i<window_maxindex_; ++i) {
        window_normalization_constant_ += is_hierarchical_ ? (alpha_h[0][i] + alpha_h[1][i]) : alpha[i];
    }
}
...
{
    check_training();
    std::size_t dimension_output = shared_parameters->dimension.get() - shared_parameters->dimension_input.get();
    ...
    if (parameters.regression_estimator.get() == HMM::RegressionEstimator::Likeliest) {
        states[results.likeliest_state].likelihood(observation_input);
        states[results.likeliest_state].regression(observation_input, predicted_output);
        return;
    }
    
    std::size_t clip_min_state = (parameters.regression_estimator.get() == HMM::RegressionEstimator::Full) ? 0 : window_minindex_;
    std::size_t clip_max_state = (parameters.regression_estimator.get() == HMM::RegressionEstimator::Full) ? parameters.states.get() : window_maxindex_;
    double normalization_constant = (parameters.regression_estimator.get() == HMM::RegressionEstimator::Full) ? 1.0 : window_normalization_constant_;
    ...
    for (std::size_t i=clip_min_state; i<clip_max_state; ++i) {
        ...
        for (std::size_t d = 0; d < dimension_output; ++d)
        {
            if (is_hierarchical_) {
                ...
                results.output_variance[d] += (alpha_h[0][i] + alpha_h[1][i]) * (alpha_h[0][i] + alpha_h[1][i]) * states[i].results.output_variance[d] / normalization_constant;
            } else {
                predicted_output[d] += alpha[i] * tmp_predicted_output[d] / normalization_constant;
                results.output_variance[d] += alpha[i] * alpha[i] * states[i].results.output_variance[d] / normalization_constant;
            }
        }
    }
}
...
void xmm::SingleClassHMM::updateResults()
{
    ...
    for (int i=window_minindex_; i<window_maxindex_; ++i) {
        if (is_hierarchical_)
            results.progress += (alpha_h[0][i] + alpha_h[1][i] + alpha_h[2][i]) * i / window_normalization_constant_;
        else
...
Json::Value xmm::SingleClassHMM::toJson() const
{
    ...
    root["states"].resize(static_cast<Json::ArrayIndex>(parameters.states.get()));
...
void xmm::SingleClassHMM::updateExitProbabilities(float *exitProbabilities)
{
    ...
    if (exitProbabilities == NULL) {
        ...
        exit_probabilities_[numStates-1] = DEFAULT_EXITPROBABILITY_LAST_STATE();
    } else {
        exit_probabilities_.resize(numStates, 0.0);
        for (int i=0 ; i < numStates ; i++)
            try {
...
//}
//
//xmm::HMM xmm::SingleClassHMM::extractSubmodel(std::vector<std::size_t>& columns) const
//{
//    check_training();
...
//    }
//    for (std::size_t i=dimension_-dimension_input_, j=0; i<dimension_; ++i, ++j) {
//        columns[i] = j;
//    }

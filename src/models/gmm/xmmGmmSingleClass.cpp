...
SingleClassProbabilisticModel(p, root)
{
    ...
    json2vector(root["mixture_coeffs"], mixture_coeffs, parameters.gaussians.get());
...
Json::Value xmm::SingleClassGMM::toJson() const
{
    ...
    root["components"].resize(static_cast<Json::ArrayIndex>(parameters.gaussians.get()));
...
//}
//
//xmm::SingleClassGMM xmm::SingleClassGMM::extractSubmodel(std::vector<std::size_t>& columns) const
//{
//    check_training();
...
void xmm::SingleClassGMM::allocate()
{
    ...
    beta.resize(parameters.gaussians.get());
    components.assign(parameters.gaussians.get(),
...
double xmm::SingleClassGMM::obsProb(const float* observation, int mixtureComponent) const
{
    double p(0.);
    
    if (mixtureComponent < 0) {
        for (mixtureComponent=0; mixtureComponent<parameters.gaussians.get(); mixtureComponent++) {
        ...
        p = mixture_coeffs[mixtureComponent] * components[mixtureComponent].likelihood(observation);
    }
                                          ...
                                          int mixtureComponent) const
{
    ...
    if (mixtureComponent < 0) {
        ...
        p = mixture_coeffs[mixtureComponent] * components[mixtureComponent].likelihood_input(observation_input);
    }
                                            ...
                                            int mixtureComponent) const
{
    ...
    if (mixtureComponent < 0) {
        ...
        p = mixture_coeffs[mixtureComponent] * components[mixtureComponent].likelihood_bimodal(observation_input, observation_output);
    }
...
void xmm::SingleClassGMM::initCovariances_fullyObserved(TrainingSet *trainingSet)
{
    ...
    for (auto phrase_it = trainingSet->begin(); phrase_it != trainingSet->end(); phrase_it++) {
        std::size_t step = phrase_it->second->size() / parameters.gaussians.get();
        std::size_t offset(0);
        for (int n=0; n<parameters.gaussians.get(); n++) {
            for (int t=0; t<step; t++) {
                for (int d1=0; d1<dimension; d1++) {
                    gmeans[n*dimension+d1] += phrase_it->second->getValue(offset+t, d1);
                    if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                        for (int d2=0; d2<dimension; d2++) {
                            components[n].covariance[d1*dimension+d2] += phrase_it->second->getValue(offset+t, d1) * phrase_it->second->getValue(offset+t, d2);
                        }
                    } else {
    ...
    for (int n=0; n<parameters.gaussians.get(); n++) {
        for (int d1=0; d1<dimension; d1++) {
            if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
                for (int d2=0; d2<dimension; d2++)
                    components[n].covariance[d1*dimension+d2] -= gmeans[n*dimension+d1]*gmeans[n*dimension+d2];
            } else {
                components[n].covariance[d1] -= gmeans[n*dimension+d1] * gmeans[n*dimension+d1];
            }
        }
    }
}
...
double xmm::SingleClassGMM::emAlgorithmUpdate(TrainingSet *trainingSet)
{
    ...
    for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); ++it) {
        ...
        for (int t=0; t<T; t++) {
            ...
            {
                if (shared_parameters->bimodal.get()) {
                    p[c][tbase+t] = obsProb_bimodal(it->second->getPointer_input(t),
                                                    it->second->getPointer_output(t),
                                                    c);
                } else {
    ...
    for (int c=0; c<parameters.gaussians.get(); c++) {
        for (int d=0; d<dimension; d++) {
            ...
            for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); ++it) {
                std::size_t T = it->second->size();
                for (int t=0; t<T; t++) {
                    components[c].mean[d] += p[c][tbase+t] * it->second->getValue(t, d);
                }
                tbase += T;
            }
            ...
            for (int d1=0; d1<dimension; d1++) {
                for (int d2=d1; d2<dimension; d2++) {
                    ...
                    for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); ++it) {
                        std::size_t T = it->second->size();
                        for (int t=0; t<T; t++) {
                            components[c].covariance[d1 * dimension + d2] += p[c][tbase+t]
                            * (it->second->getValue(t, d1) - components[c].mean[d1])
                            * (it->second->getValue(t, d2) - components[c].mean[d2]);
                        }
                        tbase += T;
                    }
                    components[c].covariance[d1 * dimension + d2] /= E[c];
                    if (d1 != d2)
                        components[c].covariance[d2*dimension+d1] = components[c].covariance[d1*dimension+d2];
                }
            }
        }
    }
        ...
        for (int c=0; c<parameters.gaussians.get(); c++) {
            for (int d1=0; d1<dimension; d1++) {
                ...
                for (auto it = trainingSet->cbegin(); it != trainingSet->cend(); ++it) {
                    std::size_t T = it->second->size();
                    for (int t=0; t<T; t++) {
                        float value = (it->second->getValue(t, d1) - components[c].mean[d1]);
...
void xmm::SingleClassGMM::initParametersToDefault(std::vector<float> const& dataVariance)
{
    int dimension = static_cast<int>(shared_parameters->dimension.get());
    ...
    for (int i=0 ; i<dataVariance.size() ; i++) {
        current_regularization[i] = std::max(parameters.absolute_regularization.get(),
                                             parameters.relative_regularization.get() * dataVariance[i]);
    }
    for (int c=0; c<parameters.gaussians.get(); c++) {
        if (parameters.covariance_mode.get() == GaussianDistribution::CovarianceMode::Full) {
            components[c].covariance.assign(dimension*dimension, parameters.relative_regularization.get()/2.);
        } else {
            components[c].covariance.assign(dimension, parameters.absolute_regularization.get()/2.);
        }
                                     ...
                                     std::vector<float>& predicted_output)
{
    ...
    for (int c=0; c<parameters.gaussians.get(); c++) {
        ...
        for (int d = 0; d < dimension_output; ++d)
        {
            ...
            results.output_variance[d] += beta[c] * beta[c] * components[c].output_variance[d];
        }
    }
}

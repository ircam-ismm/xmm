...
void xmm::SingleClassHMM::initParametersToDefault(
  ...
  if (parameters.transition_mode.get() == HMM::TransitionMode::Ergodic) {
    setErgodic();
  } else if (parameters.transition_mode.get() ==
...
void xmm::SingleClassHMM::initMeansWithAllPhrases(TrainingSet* trainingSet) {
  ...
  for (int n = 0; n < numStates; n++)
    for (int d = 0; d < dimension; d++) states[n].components[0].mean[d] = 0.0;
  ...
  for (int n = 0; n < numStates; n++) {
    for (int d1 = 0; d1 < dimension; d1++) {
      if (parameters.covariance_mode.get() ==
        ...
        for (int d2 = 0; d2 < dimension; d2++)
          states[n].components[0].covariance[d1 * dimension + d2] -=
              othermeans[n * dimension + d1] * othermeans[n * dimension + d2];
      } else {
        states[n].components[0].covariance[d1] -=
            othermeans[n * dimension + d1] * othermeans[n * dimension + d1];
      }
    }
...
void xmm::SingleClassHMM::setConstrained() {
  unsigned int numStates = parameters.states.get();
  std::vector<bool> priorConstraints = parameters.authorizedPrior.get();
  std::vector<bool> transitionConstraints =
      parameters.authorizedTransitions.get();
  if ((priorConstraints.size() == prior.size()) &&
      (transitionConstraints.size() == transition.size())) {
    double norm_prior = 0.0;
    for (int i = 0; i < numStates; i++) {
      prior[i] = priorConstraints[i] ? 1.0 : 0.0;
      norm_prior += prior[i];
      double norm_const(0.0);
      for (int j = 0; j < numStates; j++) {
        transition[i * numStates + j] =
            transitionConstraints[i * numStates + j] ? 1.0 : 0.0;
        norm_const += transition[i * numStates + j];
      }
      for (int j = 0; j < numStates; j++) {
        transition[i * numStates + j] /= norm_const;
      }
    }
    for (int i = 0; i < numStates; i++) {
      prior[i] /= norm_prior;
  ...
  if (parameters.transition_mode.get() == HMM::TransitionMode::LeftRight) {
    ...
    for (int i = 0; i < numStates; i++) {
      if (shared_parameters->bimodal.get()) {
        if (observation_output)
          alpha[i] = prior[i] *
                     states[i].obsProb_bimodal(observation, observation_output);
        else
  ...
  for (int j = 0; j < numStates; j++) {
    ...
    if (parameters.transition_mode.get() == HMM::TransitionMode::LeftRight) {
      ...
      if (j > 0) {
        ...
        alpha[0] +=
            previous_alpha_[numStates - 1] * transition[numStates * 2 - 1];
      }
    } else {
      for (int i = 0; i < numStates; i++) {
  ...
  for (int i = 0; i < numStates; i++) {
    ...
    if (parameters.transition_mode.get() == HMM::TransitionMode::LeftRight) {
      if (shared_parameters->bimodal.get()) {
        if (observation_output)
          beta_[i] +=
              transition[i * 2] * previous_beta_[i] *
              states[i].obsProb_bimodal(observation, observation_output);
        else
      ...
      if (i < numStates - 1) {
        if (shared_parameters->bimodal.get()) {
          if (observation_output)
            beta_[i] +=
                transition[i * 2 + 1] * previous_beta_[i + 1] *
                states[i + 1].obsProb_bimodal(observation, observation_output);
          else
      ...
      for (int j = 0; j < numStates; j++) {
        if (shared_parameters->bimodal.get()) {
          if (observation_output)
            beta_[i] +=
                transition[i * numStates + j] * previous_beta_[j] *
...
void xmm::SingleClassHMM::baumWelch_backward_update(
  ...
  for (int i = 0; i < numStates; i++) {
    ...
    if (parameters.transition_mode.get() == HMM::TransitionMode::LeftRight) {
      beta_[i] +=
          transition[i * 2] * previous_beta_[i] * observation_likelihoods[i];
      if (i < numStates - 1) {
        beta_[i] += transition[i * 2 + 1] * previous_beta_[i + 1] *
    ...
    for (unsigned int i = 0; i < numStates; i++) {
      if (shared_parameters->bimodal.get()) {
        observation_probabilities[t * numStates + i] =
            states[i].obsProb_bimodal(currentPhrase->getPointer_input(t),
  ...
  for (int t = 0; t < T; t++) {
    for (int i = 0; i < numStates; i++) {
      gamma_sequence_[phraseIndex][t * numStates + i] =
          alpha_seq_[t * numStates + i] * beta_seq_[t * numStates + i] / ct[t];
    }
  }
    ...
    for (int i = 0; i < numStates; i++) {
      ...
      if (parameters.gaussians.get() == 1) {
        ...
        for (int c = 0; c < parameters.gaussians.get(); c++) {
          ...
          gamma_sequence_per_mixture_[phraseIndex][c][t * numStates + i] =
              gamma_sequence_[phraseIndex][t * numStates + i] * oo;
          norm_const += oo;
        }
      }
  ...
  if (parameters.transition_mode.get() == HMM::TransitionMode::LeftRight) {
    for (int t = 0; t < T - 1; t++) {
      for (int i = 0; i < numStates; i++) {
        ...
        if (i < numStates - 1) {
          epsilon_sequence_[phraseIndex][t * 2 * numStates + i * 2 + 1] =
      ...
      for (int i = 0; i < numStates; i++) {
        for (int j = 0; j < numStates; j++) {
          epsilon_sequence_[phraseIndex][t * numStates * numStates +
                                         i * numStates + j] =
              alpha_seq_[t * numStates + i] * transition[i * numStates + j] *
              beta_seq_[(t + 1) * numStates + j];
          epsilon_sequence_[phraseIndex]
                           [t * numStates * numStates + i * numStates + j] *=
          ...
          for (int d1 = 0; d1 < dimension; d1++) {
            if (parameters.covariance_mode.get() ==
              ...
              for (int d2 = d1; d2 < dimension; d2++) {
                states[i].components[c].covariance[d1 * dimension + d2] +=
          ...
          if (parameters.covariance_mode.get() ==
            ...
            for (int d2 = d1; d2 < dimension; d2++) {
              states[i].components[c].covariance[d1 * dimension + d2] /=
                  gamma_sum_per_mixture_[i * numGaussians + c];
              if (d1 != d2)
                states[i].components[c].covariance[d2 * dimension + d1] =
                    states[i].components[c].covariance[d1 * dimension + d2];
            }
          } else {
            states[i].components[c].covariance[d1] /=
...
void xmm::SingleClassHMM::reset() {
  ...
  if (is_hierarchical_) {
    for (int i = 0; i < 3; i++) alpha_h[i].resize(parameters.states.get(), 0.0);
    alpha.clear();
...
void xmm::SingleClassHMM::regression(
  ...
  results.output_covariance.assign((parameters.covariance_mode.get() ==
                                    GaussianDistribution::CovarianceMode::Full)
  ...
  unsigned int clip_min_state =
      (parameters.regression_estimator.get() == HMM::RegressionEstimator::Full)
          ? 0
          : window_minindex_;
  unsigned int clip_max_state =
      (parameters.regression_estimator.get() == HMM::RegressionEstimator::Full)
          ? parameters.states.get()
          : window_maxindex_;
    ...
    for (unsigned int d = 0; d < dimension_output; ++d) {
      if (is_hierarchical_) {
        ...
        if (parameters.covariance_mode.get() ==
          ...
          for (int d2 = 0; d2 < dimension_output; ++d2)
            results.output_covariance[d * dimension_output + d2] +=
                ...
                (alpha_h[0][i] + alpha_h[1][i]) *
                states[i].results.output_covariance[d * dimension_output + d2] /
                normalization_constant;
        } else {
          results.output_covariance[d] +=
              (alpha_h[0][i] + alpha_h[1][i]) *
              (alpha_h[0][i] + alpha_h[1][i]) *
              states[i].results.output_covariance[d] / normalization_constant;
        }
      } else {
        ...
        if (parameters.covariance_mode.get() ==
          ...
          results.output_covariance[d] +=
              alpha[i] * alpha[i] * states[i].results.output_covariance[d] /
              normalization_constant;
        }
      }
    }
...
void xmm::SingleClassHMM::updateResults() {
  ...
  for (int i = window_minindex_; i < window_maxindex_; ++i) {
    if (is_hierarchical_)
      results.progress += (alpha_h[0][i] + alpha_h[1][i] + alpha_h[2][i]) * i /
                          window_normalization_constant_;
    else

...
class ConcurrentHMM : public ConcurrentModels< HMM<ownData>, Phrase<ownData, 1> > {
public:
    typedef typename  map<Label, HMM<ownData> >::iterator model_iterator;
    typedef typename  map<Label, HMM<ownData> >::const_iterator const_model_iterator;
    typedef typename  map<int, Label>::iterator labels_iterator;
    
    bool updateWithEstimatedObservation;
    
    ConcurrentHMM(TrainingSet< Phrase<ownData, 1> > *_globalTrainingSet=NULL)
    : ConcurrentModels< HMM<ownData>, Phrase<ownData, 1> >(_globalTrainingSet)
    {
    }
